...
namespace Ciber_Turtle.Audio
{
	...
	public class SFX : MonoBehaviour
	{
		...
		public SFXSound PlaySound(string name, float volume = 0, float pitch = 0)
		{
			...
			if (sfx != null)
			{
				PlaySoundRaw(Util.GetRandomItem(sfx.sounds), sfx.volume.randomValue * Convert.ToInt16(addWhenOveride) + volume, sfx.pitch.randomValue * Convert.ToInt16(addWhenOveride) + pitch, sfx.alt);
			}
			else
			{
				switch (errorMode)
